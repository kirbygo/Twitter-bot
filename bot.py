...
class BotStreamListener(tweepy.StreamListener):
    ...
    def on_status(self, tweet):
        ...
        if (not tweet.retweeted) and ('RT @' not in tweet.text) and ('en base a' in tweet.text) and ('bot' not in tweet.text) and ('@en_base_a_bot' not in tweet.text) and (tweet.user.followers_count >= 10000):
            reply = None
            try:
